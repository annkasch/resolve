...
class Trainer():
    def __init__(self, model, dataset):
        ...
        self.training_epochs = int(self.dataset.config_file["cnp_settings"]["training_epochs"])
    ...
    def fit(self, optimizer, writer, dataset_val):
        ...
        for it_epoch in range(self.training_epochs):
            y_preds = []
            y_trues = []
            total_logprob_loss = 0.0
            total_bce_loss = 0.0
            ...
            for b, batch in tqdm(enumerate(dataloader), total=math.ceil(len(dataloader) / self.dataset._batch_size), desc=f"Training Epoch {it_epoch+1}/{self.training_epochs}"):
                ...
                # Use the actual PyTorch tensors
                y_pred = mu[0].squeeze()                      # shape: (batch,)
                y_true = batch_formatted.target_y[0].squeeze()
                ...
                if self.is_binary:
                    bce_loss = bce(mu, batch_formatted.target_y)
                    #writer.add_scalar('Loss/BCE/Train', bce_loss.item(), it_step)
                
                loss = -log_prob.mean()
                
                loss = loss + self.penalty_loss_fp(y_pred, y_true)
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
            ...
            self.metrics_train = self.log_metrics(writer, y_true, y_pred, avg_logprob_loss, it_step, self.is_binary, leg="Train")   
            ...
            self.metrics_eval = self.log_metrics(writer, y_true_val, y_pred_val, loss_logprob, it_step, self.is_binary, leg="Eval")
            self.metrics_eval['BCE'] = loss_bce if self.is_binary else None
            writer.add_scalar('Loss/BCE/Eval', loss_bce, it_step)
