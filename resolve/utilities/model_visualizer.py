    ...
    def plot_projection(self,x_data, y_data, with_prediction=False, grid_steps=20):
                ...
                for f in range(nrows):
                    ...
                    for keep_axis in range(ncols):
                        ax = axes[f][keep_axis]
                        if with_prediction:
                            x, y_mean, y_1sigma, y_2sigma, y_3sigma = self.get_marginalized_single_draw(np.array(x_data[f]), np.array(y_prediction[f]), keep_axis=keep_axis, grid_steps=grid_steps)
                        ...
                        x, y, [y_low, y_high],_,_ = self.get_marginalized_single_draw(np.array(x_data[f]), np.array(y_data[f]), keep_axis=keep_axis, grid_steps=grid_steps)
