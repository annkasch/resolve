...
class HCTargetAttnNP(nn.Module):
    def __init__(self, d_theta, d_phi, d_y, d_model=32, encoder_sizes=[128,128], theta_embed_dim=32, n_heads=4, mode = 'film'):
        ...
        #self.attn1 = GlobalContextAttentionDual(d_model, n_heads=n_heads, out_dim=d_model)
        
        # Target encoder -> z_t
        self.tgt_enc = TargetEncoder(
            theta_in_dim=d_theta, phi_dim=d_phi, r_dim=d_model,
            hidden=encoder_sizes, out_dim=d_model
        )
    ...
    def forward(self, query_theta, query_phi, context_theta, context_phi, context_y, qry_theta_cell=None, return_ctx_for_write=True, **kwargs):
        ...
        r_all = self.tgt_dec(query_theta, query_phi, R_ctx, context_y)
        #R_ctx = self.ctx_enc2(context_theta, context_phi, context_y) 
        # 2) Build per-target query R^{(t)}
        #R_t = self.tquery(query_theta, query_phi)                 # (B,Nt,D)
        #R_t = self.tgt_dec(query_theta, query_phi) 
        ...
        if qry_theta_cell is not None:
            ...
            if self.memory.pos_mask.any():
                ...
                R_ctx_pos=r_pos
                #R_ctx_neg=r_neg
                R_ctx_neg = R_ctx[~wS].unsqueeze(0).repeat(B, 1, 1)
                
                wS_pos=torch.ones(R_ctx_pos.shape[:-1]).to(dev)
                wB_neg=torch.zeros(R_ctx_neg.shape[:-1]).to(dev)

                R_ctx = torch.cat([R_ctx_pos, R_ctx_neg], dim=-1)
                w = torch.cst([wS_pos,wB_neg], dim=-1)

                r_pos, r_neg, r_all = self.attn1(Q_src=R_t, K_src=R_ctx, V_src=R_ctx, wS=w)  # (B,Nt,D)

            else:
                r_pos, r_neg, r_all = self.attn1(Q_src=R_t, K_src=R_ctx, V_src=R_ctx, wS=wS)
        else:
            r_pos, r_neg, r_all = self.attn1(Q_src=R_t, K_src=R_ctx, V_src=R_ctx, wS=wS)
        """
        loss =0 
        ...
        #r_pos, r_neg, loss = self.attn(Q_src=R_t, K_src_pos=R_ctx_pos, K_src_neg=R_ctx_neg)
        ...
        #r_neg = self.attn(Q_src=R_t, K_src=R_ctx, V_src=R_ctx)  # (B,Nt,D)
        ...
        #print(loss, "cos mean:", cos.mean().item(), "cos p95:", cos.quantile(0.95).item())
        #print("atten pos", r_pos.mean().item(), "atten neg", r_neg.mean().item())
        ...
        z_t = self.tgt_enc(query_theta, query_phi, r_all)  # (B,Nt,D)
