...
class HCTargetAttnLNP(nn.Module):
    ...
    def forward(self, theta_c, phi_c, y_c, theta_t, phi_t, mask_c=None, beta=1.0, train_mode=True):
        B, Nc, _ = phi_c.shape
        if mask_c is None:
        ...
        if self.use_theta_prior:
            theta_bar = masked_mean(theta_c, mask_c, dim=1)  # (B,d_theta)
        ...
        wS_ctx = y_c.squeeze(-1).clamp(0,1)           # (B,Nc)
