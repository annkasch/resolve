...
import logging
from pymc.distributions.continuous import TruncatedNormal
...
class PCEMultiFidelityModel:
    def __init__(self, trainings_data, priors, parameters, degree=None):
        ...
        self.set_table()
        self.table_flat_dict[f"dataset_{self.fidelities[0]}"] = len(self.trainings_data[self.fidelities[0]][0][0])
        for i,f in enumerate(self.fidelities[1:]):
            ...
            self.table_flat_dict[f"dataset_{self.fidelities[i]}"] = len(self.indices[f][0])
    ...
    def _generate_basis(self, x_data, degree):
        ...
        degree_new = degree
        ntimes = 1
        while (n_samples < ntimes*comb(n_dim + degree_new, degree_new)):
        ...
        if degree_new != degree:
            print(
                ...
                f"  -> Required: at least {ntimes * comb(n_dim+degree, degree)} samples\n"
    ...
    def _add_fidelity(self, model, fidelity, y_prev_pred_full):
        ...
        # Scaling factor (optimized by minizing MSE of observed data y_lf and y_hf)
        try:
        ...
        rho_i = observed[hf_indices_list] / observed_fminus1[lf_indices_list]
        sigma_rho_i = np.mean(np.sqrt(
            (np.std(observed[hf_indices_list]) / observed_fminus1[lf_indices_list])**2
            + (rho_i * self.priors[self.fidelities[fidelity-1]]["sigma_y"] / observed_fminus1[lf_indices_list])**2
        ))
        print(f"Optimized mu_rho: {mu_rho:.5f}, std {sigma_rho_i:.5f}")
        self.priors[self.fidelities[fidelity]]["sigma_rho"]=truncate(sigma_rho_i,5)
        ...
        y_prev_pred_subset = pm.Deterministic(f"y_prev_pred_subset_{self.fidelities[fidelity]}",pm.math.stack([y_prev_pred_full[i] for i in subset_indices]))
        ...
        #log_mu = pm.math.log(pm.math.maximum(1e-6, y_pred))  # Prevent log(0)
        #y_likeli = pm.LogNormal(f"y_likeli_{self.fidelities[fidelity]}", mu=log_mu, sigma=sigma, observed=self.trainings_data[self.fidelities[fidelity]][1][hf_indices_list])
        #y_likeli = TruncatedNormal(f"y_likeli_{self.fidelities[fidelity]}", mu=y_pred, sigma=sigma, lower=0, observed=self.trainings_data[self.fidelities[fidelity]][1][hf_indices_list])
        ...
        """
        if fidelity == len(self.fidelities) - 1:
            self.s = 1e7  # star formation rate density (from Madau & Dickinson, 2014)
            self.epsilon_r = 1.57133427e-6/0.005  # from your comment
            self.y_obs = np.array([17]) # observed number of events
            # Compute expected number of observed events
            scaled_rate = pm.Deterministic("expected_event_rate", self.s * self.epsilon_r * y_pred)

            logL = pm.math.sum(
                self.y_obs * pm.math.log(pm.math.maximum(scaled_rate, 1e-10)) 
                - scaled_rate 
                - gammaln(self.y_obs + 1)
            )
            pm.Deterministic("total_log_likelihood", logL)

            # Optional: include likelihood node explicitly for PyMC tracking
            #pm.Potential("poisson_logp", logL)
        """
        return y_pred
    ...
    def set_table(self):
        ...
        self.table_flat_dict[f"polynomial_order_{self.fidelities[0]}"] = self.degree[0] 
        for f in self.fidelities[1:]:
            self.table_flat_dict[f"polynomial_order_delta_{f}"] = self.degree[self.fidelities.index(f)]        
